...
export const zodlistProfessionalServicesQuerySortDirectionDefault = "asc";
export const zodlistProfessionalServicesQueryActiveOnlyDefault = true;

export const zodlistProfessionalServicesQueryParams = zod.object({
  page: zod
    .number()
    .min(zodlistProfessionalServicesQueryPageMin)
    .default(zodlistProfessionalServicesQueryPageDefault)
    .describe("Número da página atual (começa em 1)"),
  limit: zod
    .number()
    .min(zodlistProfessionalServicesQueryLimitMin)
    .max(zodlistProfessionalServicesQueryLimitMax)
    .default(zodlistProfessionalServicesQueryLimitDefault)
    .describe("Quantidade de itens por página (máximo 100)"),
  sortBy: zod.string().optional().describe("Campo para ordenação (opcional)"),
  sortDirection: zod
    .enum(["asc", "desc"])
    .default(zodlistProfessionalServicesQuerySortDirectionDefault)
    .describe("Direção da ordenação: asc (crescente) ou desc (decrescente)"),
  activeOnly: zod
    .boolean()
    .default(zodlistProfessionalServicesQueryActiveOnlyDefault),
});
...
export const zodupdateServiceProfessionalBody = zod.object({
  services: zod.array(
    zod.object({
      ...
      price: zod
        .number()
        .min(zodupdateServiceProfessionalBodyServicesItemPriceMin),
      duration: zod
        .number()
        .min(zodupdateServiceProfessionalBodyServicesItemDurationMin),
      linked: zod.boolean(),
    }),
  ),
});
