...
export const zodServiceType = zod.enum([
  "CORTE",
  "BARBA",
  "SOBRANCELHA",
  "ESTETICA",
...
export const zodcreateServiceBody = zod.object({
  name: zod
    .string()
    .min(zodcreateServiceBodyNameMin)
    .max(zodcreateServiceBodyNameMax),
  type: zodServiceType,
...
export const zodlistServicesQuerySortDirectionDefault = "asc";
export const zodlistServicesQueryNameMax = 100;
export const zodlistServicesQueryCategoryMax = 50;

export const zodlistServicesQueryParams = zod.object({
  page: zod
    .number()
    .min(zodlistServicesQueryPageMin)
    .default(zodlistServicesQueryPageDefault)
    .describe("Número da página atual (começa em 1)"),
  limit: zod
    .number()
    .min(zodlistServicesQueryLimitMin)
    .max(zodlistServicesQueryLimitMax)
    .default(zodlistServicesQueryLimitDefault)
    .describe("Quantidade de itens por página (máximo 100)"),
  sortBy: zod.string().optional().describe("Campo para ordenação (opcional)"),
  sortDirection: zod
    .enum(["asc", "desc"])
    .default(zodlistServicesQuerySortDirectionDefault)
...
export const zodupdateServiceByIdBody = zod.object({
  name: zod
    .string()
    .min(zodupdateServiceByIdBodyNameMin)
    .max(zodupdateServiceByIdBodyNameMax)
    .optional(),
  description: zod
    .string()
    .max(zodupdateServiceByIdBodyDescriptionMax)
    .optional(),
  defaultPrice: zod
    .number()
    .min(zodupdateServiceByIdBodyDefaultPriceMin)
    .optional(),
...
export const zodtoggleServiceStatusByIdResponse = zod
  .enum(["null"])
  .nullable()
  .describe("Status do serviço atualizado com sucesso.");
