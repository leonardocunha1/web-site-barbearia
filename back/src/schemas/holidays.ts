  ...
  .object({
    date: z
      ...
      .datetime({
        offset: true,
        message:
          'Data inválida. Utilize o formato ISO 8601 com timezone (ex: 2023-12-25T00:00:00-03:00)',
      })
...
export const deleteHolidayParamsSchema = z
  .object({
    holidayId: z
      .string()
...
export const listHolidaysResponseSchema = z
  .object({
    holidays: z
      .array(
        z.object({
          id: z.string().uuid({ message: 'ID do feriado inválido' }).describe('ID do feriado'),
          date: z
            .string()
            .datetime({ message: 'Formato de data inválido' })
            .describe('Data no formato ISO 8601'),
          reason: z.string().describe('Motivo do feriado'),
          professionalId: z
            .string()
