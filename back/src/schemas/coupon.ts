...
export const couponSortFieldSchema = z
  .enum(['code', 'type', 'scope', 'createdAt', 'startDate', 'endDate', 'active', 'uses'], {
    errorMap: () => ({
      message:
        'Campo de ordenação inválido. Valores válidos: code, type, scope, createdAt, startDate, endDate, active, uses',
    }),
  })
 ...
 */
export const couponSortOrderSchema = z
  .enum(['asc', 'desc'], {
    errorMap: () => ({
      message: 'Ordem de ordenação inválida. Use "asc" ou "desc"',
    }),
  })
...
export const createCouponBodySchema = z
  .object({
    ...
    minBookingValue: z.number().positive({ message: 'Valor mínimo deve ser positivo' }).optional(),
    serviceId: z.string().uuid({ message: 'ID do serviço inválido' }).optional(),
    professionalId: z.string().uuid({ message: 'ID do profissional inválido' }).optional(),
  })
...
export const updateCouponParamsSchema = z
  .object({
    couponId: z.string().uuid({ message: 'ID do cupom inválido' }),
  })
  ...
  .object({
    ...
    type: z
      .enum(['PERCENTAGE', 'FIXED', 'FREE'], {
        errorMap: () => ({
          message: 'Tipo inválido. Valores válidos: PERCENTAGE, FIXED, FREE',
        }),
      })
      .optional(),
    value: z.number().min(0, { message: 'Valor não pode ser negativo' }).optional(),
    scope: z
      .enum(['GLOBAL', 'SERVICE', 'PROFESSIONAL'], {
        errorMap: () => ({
          message: 'Escopo inválido. Valores válidos: GLOBAL, SERVICE, PROFESSIONAL',
        }),
      })
      .optional(),
    ...
    minBookingValue: z
      ...
      .nullable()
      .optional(),
    serviceId: z.string().uuid({ message: 'ID do serviço inválido' }).nullable().optional(),
    professionalId: z
      .string()
